...
angular.module("ps.search.item", ["ps.dataService.donwloadManager", "ps.dataService.item", "ps.dataService.tag", "ps.player", "ps.config.route", "ngTagsInput", "ngStorage"]).config(["$routeProvider", "commonKey", function ($routeProvider, commonKey) {
    $routeProvider.when("/items", {
        ...
        resolve: { items: ["itemService", "SearchItemCache", function (itemService, SearchItemCache) {
        ...
        save: {
            value: function save() {
                ...
                return this.podcastService.patch(podcastToUpdate).then(function (patchedPodcast) {
                    return _.assign(_this.podcast, patchedPodcast);
                }).then(function () {
                    return _this.$scope.$emit("podcastEdition:save");
                });
            }
        },
        deletePodcast: {
            value: function deletePodcast() {
                ...
                return this.podcastService.deletePodcast(this.podcast).then(function () {
                    return _this.$location.path("/podcasts");
                });
            }
        }
    });
...
var PodcastDetailCtrl = (function () {
    function PodcastDetailCtrl($scope, podcast, UpdateService) {
        ...
        this.podcastTabs = [{ heading: "Episodes", active: true }, { heading: "Edition", active: false }, { heading: "Upload", disabled: this.podcast.type !== "send" }, { heading: "Stats", active: false }];
...
angular.module("ps.podcast.details", ["ps.config.route", "ps.podcast.details", "ps.podcast.details.episodes", "ps.podcast.details.edition", "ps.podcast.details.upload", "ps.podcast.details.stats", "ps.dataService.updateService"]).config(["$routeProvider", "commonKey", function ($routeProvider, commonKey) {
